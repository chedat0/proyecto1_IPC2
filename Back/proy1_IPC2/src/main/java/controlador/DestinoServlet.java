...
public class DestinoServlet extends BaseServlet {
        ...
        try {
            if (path == null || path.equals("/")) {
                ...
                if (parts.length >= 3 && "paquetes".equals(parts[2])) {
                    ...
                    if (d == null) {
                        sendNotFound(resp, "Destino no encontrado.");
                    } else {
                        sendOk(resp, d);
                    }
                }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (d.getNombre() == null || d.getNombre().isBlank() || d.getPais() == null || d.getPais().isBlank()) {
                sendBadRequest(resp, "Nombre y País son obligatorios.");
                return;
            }
            if (destinoDAO.obtenerPorNombre(d.getNombre()) != null) {
                sendBadRequest(resp, "Ya existe un destino con ese nombre.");
                return;
            }
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            ...
            Destino existente = destinoDAO.obtenerPorId(id);
            if (existente == null) {
                sendNotFound(resp, "Destino no encontrado.");
                return;
            }
            Destino d = GSON.fromJson(req.getReader(), Destino.class);
            d.setIdDestino(id);
            d.setActivo(existente.isActivo()); 
    ...
    protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            int id = Integer.parseInt(extractIdFromPath(req.getPathInfo()));
            if (!destinoDAO.eliminar(id)) {
                sendNotFound(resp, "Destino no encontrado.");
            } else {
                sendOk(resp, Map.of("mensaje", "Destino eliminado."));
            }
        } catch (Exception e) {
            sendServerError(resp, e.getMessage());
        }
